  ...
  async findBySlug(params, userId): Promise<any> {
    try {
      ...
      if (course) {
        ...
        audios = await this.audioModel.find({ courseId: course.id });
        ...
        videos = await this.videoModel.find({ courseId: course.id });
      ...
      return { course, audios, videos: videoCategories, blogs, galleries };
    } catch (error) {
