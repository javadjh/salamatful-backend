    ...
    async createCode({ phone }) {
        try {
            if (phone) {
                ...
                if (account) {
                    const userCantGetAnotherCode = await this.codeModel.findOne({ phone });
                    if (!userCantGetAnotherCode) {
                        ...
                        return { code: 0, message: 'Your previous code has not expired yet' };
                    } else {
                        const code = this.generateCodeDigits();
                        const validDate = this.setTimeForNextCode();
                        await this.codeModel.updateOne({ phone, code: userCantGetAnotherCode.code }, { code, validDate });
                        await kavenegar.sendForgotPasswordCode({ phone, code });
                        return { time: validDate };
                    }
                } else {
                    return { code: 0, message: 'No account exists with this phone number in the database' }
                }
            } else {
                return { code: 0, message: 'Please fill the phone number field' }
            }
        } catch (error) {
            return { code: -1, message: 'Error occurred: while creating a code in the code service' }
        }
    }
    ...
    async validateCodeWithPhoneNumber({ phone, code, pass }) {
        try {
            if (phone && code && pass) {
                ...
                return { code: 0, message: 'Missing attributes: Phone and Code and Password' };
            }
        } catch (error) {
            return { code: -1, message: 'Error occurred: while trying to validate the code with the phone number' };
        }
    }
